...
(function ($) {
  ...
  async function fetchData() {
    ...
    $("#standby").text(nodeDataJson.totalResigned + nodeDataJson.totalProposed);
    ...
    const validatorBalanceData = await fetch(
      "https://explorer.xinfin.network/api/accounts/xdc0000000000000000000000000000000000000088"
    );
    const validatorBalanceDataJson = await validatorBalanceData.json();
    const validatorBalance = numberToReadable(
      validatorBalanceDataJson.balanceNumber
    );
    $("#tvl").text(validatorBalance.num);
    $("#tvltext").text(validatorBalance.text);
